        ...
        if (err) res.send(err);
          ...
          db.query("select * from crop", function (err, result2) {
            if (err) res.send(err);
              ...
              db.query(
                "select temperature, waterLevel from meka1 ORDER BY _id desc LIMIT 6",
...
/*
  ...
  console.log(result);
    ...
    */
app.get("/sql/relay1", function (req, res) {
  const body = req.body;
  const sql =
    "insert into meka1 (temperature,	humi,	co2,	waterLevel,	StatTime,	relay1,	relay2,	relay3, relay4) values (SELECT temperature FROM meka1 order by _id desc LIMIT 1, SELECT humi FROM meka1 order by _id desc LIMIT 1, SELECT co2 FROM meka1 order by _id desc LIMIT 1, SELECT waterLevel FROM meka1 order by _id desc LIMIT 1, NOW(), ?, SELECT relay2 FROM meka1 order by _id desc LIMIT 1, SELECT relay3 FROM meka1 order by _id desc LIMIT 1, SELECT relay4 FROM meka1 order by _id desc LIMIT 1)";
  var id = [req.params.id];
